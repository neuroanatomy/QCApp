...
class MyImages extends JComponent {
    ...
    public void renew() {
        ...
        initialized = false;
        volumes = new MyVolumes();
        rect = Stream.generate(() -> new Rectangle()).limit(imgList.length).toArray(Rectangle[]::new);
        repaint();
    }
                ...
                try {
                    ...
                    if (imgList[i].color) {
                        ...
                        QCApp.printStatusMessage(
                                String.format("R:%d A:%d S:%d %d", voxel.x, voxel.y, voxel.z, voxel.value));
                    }
                } catch (Exception e1) {
    ...
    private Voxel getVoxel(MyVolume vol, int[] selectedSlice, int plane, int[][] boundingBox, int x, int y)
            throws Exception {
        int z;
        int x1, y1, z1;
        int x2, y2, z2;
    ...
    {
        return drawSlice(vol, null, selectedSlice, plane, cmapindex, true, 1);
    }
...
//			}
    ...
    public void paint(Graphics g) {
        ...
        if (selectedImage == 0) {
            ...
            for (i = 0; i < bufImgList.length; i++) {
                ...
                rect[i] = new Rectangle(xoff, yoff, (int) (scale * bufImgList[i].getWidth()),
                        (int) (scale * bufImgList[i].getHeight()));
            ...
            yoff = 0;
            rect[i] = new Rectangle(xoff, yoff, (int) (scale * bufImg0.getWidth()),
                    (int) (scale * bufImg0.getHeight()));
    ...
    public int setImages(boolean init) {
        ...
        if (selectedImage == 0) {
            ...
            for (i = 0; i < imgList.length; i++) {
                ...
                if (vol.volume == null) {
                    QCApp.printStatusMessage("ERROR: Volume \"" + new File(subjectDir, volName) + "\" unavailable.");
    ...
    public void updatePositionLabel() {
        if (QCApp.positionLabel != null)
            QCApp.positionLabel
                    .setText(String.format("R:%d A:%d S:%d", selectedSlice[0], selectedSlice[1], selectedSlice[2]));
    }
